---
name: graphify
description: "any input (code, docs, papers, images, videos) to knowledge graph. Use when user asks any question about a codebase, documents, or project content - especially if c:/MyCode/4_SourceTree/SNI_GitHub/Agent_Memories/graphify-out/ exists, treat the question as a /graphify query."
trigger: /graphify
---
...
### ⚠️ FIXED OUTPUT DIRECTORY (MANDATORY)

**All graphify outputs MUST go to this fixed absolute path — never a relative `graphify-out/` inside the scanned project directory.**

| Context | Path |
|---------|------|
| Linux/bash | `/sessions/pensive-quirky-hypatia/mnt/Agent_Memories/graphify-out` |
| Windows | `C:\MyCode\4_SourceTree\SNI_GitHub\Agent_Memories\graphify-out` |

At the very start of every run, set this variable and create the directory:

```bash
GRAPHIFY_OUT="/sessions/pensive-quirky-hypatia/mnt/Agent_Memories/graphify-out"
mkdir -p "$GRAPHIFY_OUT"
```

In **every bash block** below, replace `graphify-out/` with `$GRAPHIFY_OUT/`.  
In **every Python block** below, replace `Path('graphify-out/...)` with `Path('/sessions/pensive-quirky-hypatia/mnt/Agent_Memories/graphify-out/...')`.

This rule applies to ALL steps and ALL subcommands (`--update`, `query`, `path`, `explain`, `add`, `--cluster-only`, etc.). Do NOT write output files next to the scanned project.
